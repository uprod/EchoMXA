...
EchoEditor::EchoEditor (EchoProcessor& proc)
      ...
      schematic (proc)
{
    ...
    setupSwitch (rateSwitch, "RATE", "rate", "switchRate");
    setupDial (repeatsDial, "REPEATS", "repeats");
    setupDial (decayDial,   "DECAY",   "decay");
    setupDial (pitchDial,   "PITCH",   "pitch");
    setupDial (lengthDial,  "LENGTH",  "length");

    startTimerHz (30);   // pilote FIG. 1 (train d'echos vivant) et FIG. 2
}
...
void EchoEditor::setupSwitch (Dial& d, const juce::String& labelText, const juce::String& paramID,
                             const juce::String& componentID)
{
    ...
    // Course de glisse courte, sinon chaque cran demande ~80 px de geste
    // et le commutateur semble bloque (meme reglage que chez les freres).
    ...
    d.attachment = std::make_unique<SAttach> (echoProcessor.getAPVTS(), paramID, s);
}

void EchoEditor::setupDial (Dial& d, const juce::String& labelText, const juce::String& paramID)
{
    auto& s = d.slider;
    // Drag vertical uniquement : en mode Horizontal+Vertical, JUCE somme
    // dx + (-dy), donc un geste en diagonale s'annule et le cadran semble mort.
    s.setSliderStyle (juce::Slider::RotaryVerticalDrag);
    s.setTextBoxStyle (juce::Slider::TextBoxBelow, false, 84, 16);
    s.setRotaryParameters (juce::MathConstants<float>::pi * 1.25f,
                           juce::MathConstants<float>::pi * 2.75f, true);

    // Le format des valeurs ("4.0 ms", "100 %") vient des parametres.
...
void EchoEditor::drawSheetFrame (juce::Graphics& g)
{
...
void EchoEditor::drawHeader (juce::Graphics& g)
{
    ...
    g.setColour (palette::ink);
    g.drawText ("ECHO MXA", header.withWidth (250.0f), juce::Justification::centredLeft);

    g.setFont (fonts::lettering (10.5f));
    g.setColour (palette::inkMid);
    g.drawText ("TEMPO-SYNCED MIDI NOTE ECHO",
                header.withX (header.getX() + 258.0f).withWidth (250.0f),
    ...
    g.drawText ("DWG NO. MXA-EC-01", row2.withTrimmedLeft (8.0f).withTrimmedRight (86.0f),
                juce::Justification::centredLeft);
...
void EchoEditor::resized()
{
    ...
    Dial* dials[] = { &rateSwitch, &repeatsDial, &decayDial, &pitchDial, &lengthDial };
    ...
    for (auto* d : dials)
    {
        ...
        // Le commutateur n'a pas de zone de texte : on retire la meme
        // hauteur pour que leur cercle reste a l'echelle des cadrans.
        if (d == &rateSwitch)
